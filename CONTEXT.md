...
**Track Selection Preference**：用户为同一 Media Identity 与 Content Revision 明确选择并持久化的音轨稳定身份，以及字幕轨稳定身份或 Off。再次打开相同媒体版本时，Enchron 在当前 Media Session 发布完整轨道列表后恢复仍然可用的选择；不存在的保存轨道不阻止播放，也不会通过 UI 顺序或显示名称猜测替代轨道。这项偏好不受 Persistent Viewing State 的时长和有效播放时间门槛限制，也不把只为当前 Session 授权的独立字幕文件变成长期 Media Reference。
_Avoid_：保存 UI index、按显示名称猜测轨道、内容变化后套用旧选择、把字幕 Off 当成没有偏好、为恢复选择重新打开媒体、持久化任意独立字幕来源授权
...
**Window Playback Ornament**：附着在当前 Window Playback 系统窗口底边的 PlayerControls；它随所属窗口移动，但不属于 Window 内容平面或独立 Window。Docked 与 Panorama 使用独立的 Player Control Dock。
...
**层级浏览导航**：在 Media Library 的 Library Folder 层级或来源浏览器的 Source Directory 层级中进入子层级、通过面包屑返回祖先以及后退或前进的共同界面行为。它只改变当前浏览位置，不改变层级所有权、内容位置或媒体身份；Library Folder 使用虚拟 identity 链，Remote Source 使用来源 identity 与目录路径。
_Avoid_：把 Library Folder 当成文件系统路径、把 Source Directory 当成可由 Enchron 管理的虚拟文件夹、用导航动作改变媒体身份
...
**External Subtitle File（独立字幕文件）**：不包含在媒体容器内、由可枚举的本地或远程 Source Directory 单独拥有，并在打开媒体时按同目录命名规则自动加入当前 Media Session 字幕轨列表的字幕文件。独立字幕文件不会复制来源文件，也不会变成媒体的第二个 Media Reference；单文件授权与 Photos 无法枚举同级目录时静默略过独立字幕查找。
...
_Avoid_：通用观看历史、最近播放能力、由 Media Library 决定保存规则、把 Media Format Preference 或 Track Selection Preference 并入观看进度门槛
...
**Viewing Progress Indicator**：媒体卡片进入 Gaze/Hover 状态时，在底边显示已保存 Playback Progress 的图形投影；它表达上次已知观看位置，不承诺当前媒体已经通过 Content Revision 的最终 Resume 验证。
_Avoid_：把指示器当成已验证的 Resume 承诺、用固定假定时长计算比例
...
**Presentation Transition**：从一个稳定 Playback Presentation 到另一个稳定 Presentation 的暂态。它绑定发起时的 Media Session；原先 Playing 时在平台效果前暂停，暂停失败则平台效果不开始并回滚。转换期间源与目标不能同时接受播放界面输入，只迁移同一个 renderer consumer，不进入媒体 Loading 状态。目标 Scene、Environment 或黑色周围环境、视频表面和播放控件达到成功后置条件后提交目标 Presentation，并保持 Paused；只有用户在当前可见界面显式点击 Play 才继续。目标失败时保留同一 Media Session，恢复源 Presentation 并保持 Paused。视觉交接使用能够满足这些结果的 visionOS 系统过渡，不规定 Enchron 自有的透明度曲线或源与目标准备时序。

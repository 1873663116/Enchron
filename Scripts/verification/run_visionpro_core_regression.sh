...
fixture_url=${ENCHRON_DEVICE_ACCEPTANCE_FIXTURE_URL:-}
...
if [[ -z "$fixture_url" ]]; then
    echo "Set ENCHRON_DEVICE_ACCEPTANCE_FIXTURE_URL to a credential-free media URL reachable from Vision Pro." >&2
    exit 64
fi

if [[ "$fixture_url" == *@* ]]; then
    echo "ENCHRON_DEVICE_ACCEPTANCE_FIXTURE_URL must not contain credentials because test launch configuration is retained in evidence." >&2
    exit 64
fi
...
capture_manifest() {
    ...
    local manifest_arguments=(
        ...
        --result "$result"
        --command "xcodebuild build-for-testing and test-without-building with VisionProCoreRegression"
    )
...
ENCHRON_DEVICE_ACCEPTANCE_FIXTURE_URL="$fixture_url" \
...
if (( test_exit == 0 )) && {
    (( total_test_count != expected_test_count )) ||
    (( passed_test_count != expected_test_count )) ||
    (( failed_test_count != 0 )) ||
    (( skipped_test_count != 0 )) ||
    [[ "$result" != succeeded ]]
}; then
    echo "The result bundle did not contain the complete passing test set: expected=$expected_test_count total=$total_test_count passed=$passed_test_count failed=$failed_test_count skipped=$skipped_test_count result=$result" >&2
    test_exit=65
fi
...
if (( test_exit == 0 )); then
    ...
    capture_manifest "VisionProCoreRegression did not pass: exit=$test_exit expected=$expected_test_count total=$total_test_count passed=$passed_test_count failed=$failed_test_count skipped=$skipped_test_count result=$result"
fi

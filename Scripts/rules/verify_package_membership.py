...
MODULE_DIRECTORY_SELF_CHECKS = (
    ({"name": "Playback", "path": "Modules/Playback", "exclude": []}, []),
    (
        {"name": "Playback", "path": "Modules/Playback", "exclude": ["Resources"]},
        ["Playback excludes Resources"],
    ),
    (
        {
            "name": "MediaLibrary",
...
def swift_package_json(repository_root: Path, *command: str) -> dict:
...
def package_description(repository_root: Path) -> dict:
    return swift_package_json(repository_root, "describe", "--type", "json")


def package_manifest(repository_root: Path) -> dict:
    return swift_package_json(repository_root, "dump-package")


def module_target_findings(target: dict) -> list[str]:
    name = target["name"]
    if name not in PRODUCT_TARGETS:
        return []
    findings = [f"{name} excludes {entry}" for entry in target.get("exclude") or []]
    sources = target.get("sources")
    if sources is not None:
        findings.append(
            f"{name} names {len(sources)} source entries instead of compiling its directory"
        )
    directory = f"Modules/{name}"
    if target.get("path") != directory:
        findings.append(f"{name} compiles {target.get('path')} instead of {directory}")
    return findings


def carved_module_directories(manifest: dict) -> list[str]:
    declared = {target["name"] for target in manifest["targets"]}
    findings = [
        f"{name} is absent from the package manifest"
        for name in sorted(PRODUCT_TARGETS - declared)
    ]
    for target in manifest["targets"]:
        findings += module_target_findings(target)
    return findings


def verify_module_directory_reader() -> None:
    for target, expected in MODULE_DIRECTORY_SELF_CHECKS:
        findings = module_target_findings(target)
        if findings != expected:
            raise RuntimeError(
                f"Manifest module reader misread {target['name']}: {findings!r}"
            )

    intact = [
        {"name": name, "path": f"Modules/{name}", "exclude": []}
        for name in sorted(PRODUCT_TARGETS)
    ]
...
def main() -> int:
    ...
    verify_module_directory_reader()
    description = package_description(repository_root)
    carved = carved_module_directories(package_manifest(repository_root))
    ...
    if missing or stale or import_violations or playback_app_references or carved:
        if missing:
        ...
        if carved:
            print("Module targets that do not compile their whole directory:", file=sys.stderr)
            for finding in carved:

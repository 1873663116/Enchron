#!/bin/zsh

set -euo pipefail

repository_root=${0:A:h:h:h}
source "$repository_root/Scripts/verification/enchron_artifact_paths.sh"
developer_dir=${ENCHRON_XCODE_APP:+$ENCHRON_XCODE_APP/Contents/Developer}
developer_dir=${developer_dir:-$(xcode-select -p)}
derived_data=${ENCHRON_DERIVED_DATA:-$artifact_root/DerivedData/OrganicArchitecture}
source_packages=${ENCHRON_SOURCE_PACKAGES:-$artifact_root/SourcePackages/OrganicArchitecture}
evidence_root=${ENCHRON_EVIDENCE_ROOT:-$evidence_root/organic-architecture-$(date +%Y%m%d-%H%M%S)}

mkdir -p "$evidence_root"

python3 "$repository_root/Scripts/rules/verify_playback_surface_structure.py"
DEVELOPER_DIR="$developer_dir" python3 "$repository_root/Scripts/rules/verify_package_membership.py"

build_scheme() {
    local scheme=$1
    local destination=$2
    local result_name=$3

    DEVELOPER_DIR="$developer_dir" xcodebuild \
        -project "$repository_root/Enchron.xcodeproj" \
        -scheme "$scheme" \
        -configuration Debug \
        -destination "$destination" \
        -derivedDataPath "$derived_data" \
        -clonedSourcePackagesDirPath "$source_packages" \
        -resultBundlePath "$evidence_root/$result_name.xcresult" \
        CODE_SIGNING_ALLOWED=NO \
        build
}

build_scheme Enchron 'generic/platform=visionOS' Enchron-build

echo "Xcode product builds passed; evidence: $evidence_root"

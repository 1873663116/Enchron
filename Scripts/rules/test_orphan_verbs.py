...
class OrphanVerbsTests(unittest.TestCase):
    ...
    def write_channel(self, contents: str) -> None:
        (self.repository / "Apps/Enchron/TestCommandChannel.swift").write_text(contents, encoding="utf-8")
    ...
    def test_real_repository_has_no_violations(self) -> None:
        ...
        with patch.object(checker, "REPOSITORY_ROOT", original_root), patch.object(checker, "CHANNEL_PATH", original_root / "Apps/Enchron/TestCommandChannel.swift"), patch.object(checker, "INVENTORY_PATH", original_root / "Config/reachability_operation_inventory.json"):
            self.assertEqual(checker.failures(), [])

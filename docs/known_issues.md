...
### 描述
二级进度条应该如市售的剪接软件的轨道一样,是一个比较粗的UI,这已经实现了。但是这个进度条不是像一级进度条一样的交互方式，而是出现一个中轴,这个轴就是当前视频所在的位置,而用户通过左右拖动二级进度条,可以构造一个类似于滚轴的体验,这类似于胶片在中间的一个竖线上滚动,而中间的那根中轴就是代表此刻视频所处区域。并且为上方添加随着二级进度条下方的时间刻度拉杆而变化的时间刻度，并且时间刻度完全对齐二级进度条。二级进度条的时间刻度可以被缩小，至进度条的UI长度只剩最大长度的一半，也就是实现更快速地拉动。放大时间刻度是同理的，只是进度条有最大长度限制。
而左右上一帧和下一帧的控制按钮，需要居中，并且位于最下方，也就是时间刻度放大缩小拉杆的下方。
/Users/xiongzhipeng/Downloads/二级进度条参考图片.png

### 调查结论（2026-03-06）

这条问题由**交互模型设计不符预期**与**拖动期间每帧直接触发精确 seek**两个层面共同造成。

#### 根因 A：当前实现本质上仍是传统 slider，而不是“中轴固定、轨道滚动”
...
这与期望中的“中心播放头固定、轨道在下方滚动、时间刻度与轨道严格对齐”的模型不是同一种交互。

#### 根因 B：拖动中每次 `onChanged` 都直接触发 `seek(to:)`
...
1. 先把交互骨架改成真正的“固定中轴 + 轨道滚动 + 对齐刻度 + 底部缩放与帧进按钮”。
2. 拖动时不要每次都直接驱动精确 seek。应当先更新本地拖拽状态，再做节流预览或在松手时提交。
3. 如果需要拖动中预览，预览也应区分“轻量更新”和“最终落点”，避免每个手势采样都走 `absolute+exact`。
4. 帧进/帧退应作为最终微调动作，而不是靠高频精确 seek 模拟。

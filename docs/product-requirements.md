...
- Swift Testing / XCTest 验证纯逻辑和适配边界，XCUIAutomation 验证可访问交互，`xcodebuild` 与 `.xcresult` 保存结果。Simulator 验证 UI、平台 API 和基础 RealityKit 生命周期；硬件解码、HDR/EDR、最终 Panorama、空间舒适度和性能必须在 Vision Pro 验证。完整门槛见 [`docs/acceptance/verification-system.md`](acceptance/verification-system.md)。

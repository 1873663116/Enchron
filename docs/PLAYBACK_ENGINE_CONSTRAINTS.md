...
2026-09-09 之前的规则是一道悬崖：`os_proc_available_memory()` 跌破 512 MB 就把预算退回地板。它与同一份文档里的两张表相矛盾——8K 在地板（4 帧）上每秒只显示 21 帧，在 24 帧上显示 46 帧，换来的是约 183 MB。它也没有回滞，而爬坡需要 1.5 秒，所以进入与退出会以几秒为周期来回震荡。
...
- **`PlaybackFFmpegBridge` 只按 `codec_type` 判定一条流属于哪一类**。m4a 里的封面图因此被算作视频流，带封面的音频文件的 mediaKind 是 video 而不是 audioOnly；要一个真正 audioOnly 的资产，只能选完全不带视频流的那种。见 `Scripts/rules/tests/test_fixture_registry.py`。

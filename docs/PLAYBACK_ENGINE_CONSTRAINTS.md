...
## visionOS 的系统中文字体没有轮廓表

visionOS 上 CoreText 为 zh-Hans 选出的系统字体是 `PingFangUI.ttc`（`/System/Library/PrivateFrameworks/FontServices.framework/CorePrivate/`），它的字形不在 `glyf` 或 `CFF` 里，而在 Apple 私有的 `hvgl` 表里。FreeType 打不开这个面，无论是 libass 的 CoreText provider 按路径打开，还是 `PBSubtitleSystemFontCopyChineseFallback` 把表重新打包成内存字体，都以"Error opening font"告终；日文、韩文的系统字体（Hiragino、Apple SD Gothic）仍是常规 TrueType，所以只有中文渲染成缺字方框。macOS 上同名字体仍带轮廓表，因此这条约束在 macOS 宿主的测试里看不到。

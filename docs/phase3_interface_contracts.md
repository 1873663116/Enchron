...
> 本文档定义 Endorphin Player 五个限界上下文之间的 Swift protocol 接口。这些接口是模块解耦的关键——各模块只依赖接口定义，不知道对方的内部实现。
...
## 9. FileProviding — FileBrowsing → PlaybackCore
...
| `FileProviding` | FileBrowsing/Ports | FileBrowsing 实现 | PlayerUI |

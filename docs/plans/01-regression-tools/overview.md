...
- 不改产品 Swift 源码。**偏离**：`Packages/PlaybackCore` 的 `PlaybackDebugRecorder.record` 与 `SampleBufferPlaybackSession+Diagnostics` 两处改了。前者的 `queue.sync` 与 MediaToolbox 构成锁序倒置，CI 上挂死 11 分钟，用 `sample(1)` 取到栈后改成 `queue.async`；后两处 `recordFailure` 与 `publishRendererFailure` 在写失败记录之前就发布了 `.failed`，读方拿到的是空记录。三处都是产品缺陷经由 harness 暴露，按「产品问题优先于 harness 问题」处理。锁序倒置只切断了一个方向：`PlaybackDebugRecorder` 的 `writeSnapshot`、`stop` 与 `diagnosticEvidenceJSON` 仍在 `queue.sync` 里调 `session.debugSnapshot()`，而后者要取 MediaToolbox 的互斥量。这三个入口目前的调用方都在 `PlaybackCoreController` 的 MainActor 路径上、且都在 `await` 之后，没有一条是从 MediaToolbox 回调里进来的，所以观测到的挂死随 `record` 改成 `queue.async` 消失，CI 连续三次绿。残留的条件是：主线程调 `writeDebugSnapshot()` 阻塞在队列上，队列里的工作项阻塞在互斥量上，而持有互斥量的线程正在等主线程。要彻底切断得把 `debugSnapshot()` 挪出 `queue.sync`，那要先决定 `session` 这个字段由谁保护，属于并发形状的改动，不在本计划内。`record` 从 `queue.sync` 改成 `queue.async` 同时改了一条合同：写入之后立即读取不再保证读到那次写入。`PlaybackCoreTests` 里有一个用例依赖这一点，已改为显式 flush；其余读方走 `writeSnapshot` 或 `diagnosticEvidenceJSON`，两者都仍是 `queue.sync`，因此排在已入队的写之后。

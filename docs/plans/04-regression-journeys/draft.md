...
**为什么这条旅程的判读权重与 J05 不同**（沿用 2026-08-17 裁决）：立体是佩戴者双眼才能看到的属性，截图永远只是单眼画面，判读者无法从截图判断「立体是否正确」。因此本旅程的证明重心压在机械层：**构造对了画面就对**。判断层退化为一件事——确认画面正常显示（有真实内容、在动、非黑屏纯色），不承担立体或投影正确性的判读。真正的立体感、景深舒适度归人工层。
...
2. **来源声明轴**——对上表每个片源：
   ...
   - P4 退出
3. **用户覆盖轴**（证明用户改格式真的改到渲染器，而不只是改了界面显示）：
   - 取平面片源播放 → P7(经格式编辑器改为 equirectangular180 + sideBySide)
   - 机械层：渲染器收到的投影与打包随之改变，且格式来源标记翻转为「用户覆盖」。**界面显示改变但渲染器未改变即为缺陷**——这正是覆盖轴要抓的东西
   - 再改回 flat，确认可逆
4. **Custom Angle 轴**（产品特有，无片源声明它，只能来自用户）：
   - 取 360° 片源播放 → P7 改为 customAngle，选一个非默认角度（可选角度为 180 至 360 每 10 度一档，默认 200）
   - 机械层：有效投影为 customAngle 且水平覆盖角等于所选值；改档后该值随之变化
   - 判断层：画面正常显示
5. P11。

终态：每个片源一条三点位一致记录；覆盖轴与 Custom Angle 轴各一条可逆记录。

人工层残留（附录 B）：立体感与景深是否正确、180 高分辨率片的目视确认、Custom Angle 各档的视觉合理性。每项一次，确认后由机械层长期代理。

不证明：立体正确性本身（截图不可判，归人工层）；呈现态切换（归 J07）。

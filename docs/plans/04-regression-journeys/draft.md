...
**两个家族无产品入口（已核实，非待办）**：DV P10 的样片是 DASH 清单（`.mpd`），P20 是 HLS 清单（`.m3u8`），而媒体发现的可见扩展名集合不含这两者，文件浏览器根本看不到它们。因此这两个家族**当前无法经产品入口验证**，列入附录 C 内容条件缺口，并附一个产品问题：清单格式是否应纳入发现范围（附录 D8）。不要为它们编造入口。
...
6. DV P10（DASH `.mpd`）与 P20（HLS `.m3u8`）无产品入口：清单扩展名不在媒体发现的可见集合内，文件浏览器看不到它们。两家族当前无法经产品入口验证。是否纳入发现范围见附录 D8。
...
8. **新增**：DASH `.mpd` 与 HLS `.m3u8` 清单是否应纳入媒体发现的可见扩展名。当前不可见，直接后果是 DV P10 与 P20 两个家族无法经产品入口验证（附录 C6）。纳入则需确认清单入口的浏览与播放形态；不纳入则这两个家族永久无本地入口，应从家族清单中移除而不是长期挂缺口。

...
`GridCard` 四种变体在注视下揭示的东西一致：观看进度条只在 hover 时出现（video、poster、episode 都经 `watchedProgressBar`，不直接画 `watchedEdgeProgressVisual`）；video 与 episode 的说明块经同一个 `thumbnailCaption`（scrim、底左对齐、hover 显隐）和同一个 `captionBlock`；episode 的块是标题两行、简介、`play.fill` 时长，video 的块只有体积与时长——文件名已经在缩略图下方，块里不再重复。scrim 由 `thumbnailTextScrim` 画，是文字块自己的背景，高度就是文字块高度；stops 按块高比例取自 Emby 原来 205 pt 卡片上的固定渐变（卡片 25% 处透明、50% 处 0.55、底部 0.9）落在其 160～176 pt 文字块内的位置：`Surface.textScrimClearStop`（0.1）处透明、`textScrimMidStop`（0.4）处 `textScrimOpacity`（0.55）、块底 `textScrimDenseOpacity`（0.9）。Emby 里没有文字块上方的引入段；2026-09-05 真机连续否决了「块高 × 2」与「固定 52 pt 引入段」两版，一行体积的块（约 37 pt）上方任何引入段都会让黑色明显超出文字。2026-09-05 真机否决了"角落一行体积＋半张卡的 scrim"：Files 与 Emby 的 hover 必须是同一个说明块。folder 的缩略图是平面，不需要 scrim。同一个守卫脚本钉住这些结构。
## 系统 Menu 里哪种行留得住标识符

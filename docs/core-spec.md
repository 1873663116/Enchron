...
6. 同一 Enchron App 的 macOS L2 Core scenario 使用真实媒体、真实 audio/video renderer、共享 synchronizer 与 RealityKit consumer，证明持续可见播放、可听音频、音画同步、seek、连续 seek、快进、快退、rate、pause/resume、音量、静音、close/reopen 和长时间运行。
...
macOS platform entry、RealityKit consumer 与验证编排由 Enchron 承载，不改变 PlaybackCore 的 module 边界。核心行为只有在系统节点 01–09 的相应证明成立后才能进入产品完成声明；最终 Playback Presentation、HDR 观感与交互仍由 Enchron App 和 Vision Pro 验收。

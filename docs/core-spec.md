...
PlaybackCore 接受一个 Media Source，读取容器并建立 Media Session。它解封装音视频，将视频组装为保留原始编码、时间信息和格式信令的 `CMSampleBuffer`；音频在 Apple AVFoundation 能直接接收其压缩编码时保留原始编码，否则由同一个 FFmpeg provider 解码为 renderer-ready 线性 PCM `CMSampleBuffer`。两者进入同一个 sample-buffer renderer graph。
...
Audio sample 保留时间信息、channel layout、sample-rate 与来源 codec facts，并交给 `AVSampleBufferAudioRenderer`。AVFoundation 已确认可直接接收的音频编码保留 compressed payload 与 codec configuration；不能直接接收但 FFmpeg 具有解码器的音频编码由同一个 provider 解码为交错线性 PCM。当前至少对 FLAC 执行这项转换。解码是该 provider 的 sample assembly 阶段，不形成第二条播放路线、第二个 Media Session 或失败后的 fallback。既不能直接接收也没有已接入解码能力的音频 codec 必须在 renderer 启动前以明确的 unsupported-audio-codec failure 拒绝。
...
容器内与独立文件提供的 Subtitle 轨道都具有可选择的稳定身份和带时间范围的字幕数据接口；字幕如何显示属于 Enchron。独立字幕解析失败属于可恢复的字幕来源失败，不得停止仍然有效的视频、音频或其它字幕轨。Chapter 等 container metadata 作为只读媒体事实提供，不进入 renderer graph。
...
公开控制至少包含 open、close、reopen、play、pause、seek、rate、volume、mute、audio track selection、subtitle track selection、独立字幕来源的接入与移除，以及 projection/stereo override。
...
1. 公开接口只有一条产品 sample-buffer 路径，没有 route selection、decoded pixel 或第二套 PCM 播放路径；同一个音频 provider 可以按来源 codec 组装 compressed 或 decoded-PCM sample。
2. 真实 container 可以稳定输出满足合同的 compressed video 与 renderer-ready audio sample，并正确处理 B-frame、seek、format change、end、failure 与 cleanup。
...
7. 产品 compressed video sample 的 codec configuration、color primaries、transfer function、YCbCr matrix、range 与 HDR / Dolby Vision signaling 必须在 PlaybackCore 合同验证中与 visionOS App 的 displayed-frame 观察相互校验；音频 sample 必须验证其实际 compressed 或 linear-PCM format description、时间和 channel layout。缺失、错误或仅由来源 metadata 推断都视为未通过。

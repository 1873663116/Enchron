...
| PLAY-01 / P0 | LOCAL-01、WEBDAV-01 或 SMB-01 已 playing，剩余时长 > 30 秒 | 点击 Pause、Rewind 15s、Forward 15s、Play，长按激活 Progress Bar thumb 后拖动并释放，最后 Back | lifecycle 与 label 在 playing/paused 间收敛；Progress Bar seek 后 playing；position 在容差内到目标；旧 seek epoch 不再更新 UI；同一 session；Back 后 controls 消失并完成 cleanup | 每步截图和一次 Progress Bar 拖动录屏 | product-control、product-playback、input |
...
| PLAY-04 / P1 | 短媒体接近结束 | 依次在 Stop、Loop Single Episode、Play Next 设置下播放到结束；在 Stop 下召唤 Deck，检查 Replay、前后 15 秒，打开 Precision Timeline 检查前后帧并操作 Replay | ended 仅在 active lanes 完成后发布；Stop 保留同一 Media Session 与纯黑画面，不自动显示控件；Replay 与后退可用，结尾处前进禁用；Precision Timeline 可打开，上一帧可用而下一帧禁用；Replay 在同一 Session 从零开始；repeat、next 各自执行且不出现两个 active session | 结束前后、Stop 控件状态、下一项/重播 | product-policy、product-playback |

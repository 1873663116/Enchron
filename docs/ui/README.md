...
flowchart TD
    ...
    WindowPlayback --> Overlay["Resume / Start Over · Loading"]
    WindowPlayback --> FailureAlert["System Alert\nRetry · Close"]
...
- 播放请求活动期间不挂载浏览 `TabView`，同一个 Window Playback `RealityView` 从加载开始持续挂载到 `videoVisible`。媒体打开或故障恢复等待期间在该 `RealityView` 上显示产品 `LoadingSpinner`；失败时保持该播放表面挂载，并由当前 Window 或空间 Player Controls Window 呈现 SwiftUI 系统警告框（`.alert`）中的 Retry 与 Close。画面达到 `videoVisible` 后才显示 Window Chrome 与底部 PlayerControls Ornament。Presentation Transition 不进入该加载状态。

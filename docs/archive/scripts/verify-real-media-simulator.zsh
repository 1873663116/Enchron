...
media_source=${MEDIA_SOURCE:-local}
[[ $media_source == local || $media_source == http ]] || {
    print -u2 "MEDIA_SOURCE must be local or http"
    exit 1
}
expected_provider=FFmpegCompressed
work=${TMPDIR:-/tmp}/EnchronRealMediaVerification-$media_container-$media_source
...
app=$derived_data/Build/Products/Debug-xrsimulator/Enchron.app
xcrun simctl install $device $app
if [[ $media_source == http ]]; then
...
print "PASS $media_container $media_source real media first-frame and motion verification"

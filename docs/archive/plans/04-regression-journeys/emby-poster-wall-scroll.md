...
4. **修正运行手册。** 旧的 `references/enchron.md`（已拆入 `references/product.md`）曾记"对 Emby 滚动视图发 swipeUp 导致 runner 死亡……Emby 界面一律不发合成滑动"。该禁令方向错误：Emby 界面带 identifier 的合成滑动正常工作，真正的禁令是"合成滑动一律带 identifier"。同一处记录的 `docs/plans/02-source-parity-acceptance/decision-log.md:165` 与 `decisions.tsv:26` 亦为误归因。

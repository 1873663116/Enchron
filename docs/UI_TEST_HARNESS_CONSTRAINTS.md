...
- **视图模式偏好活过 relaunch**：`MediaLibraryUIState.viewMode` 落盘在 `enchron.mediaLibrary.uiPreferences` 里，relaunch 不清它。切过视图模式的场景必须在同一场景内把它切回 grid（该控件是左右两半分别命中，复位要点左半）并验证网格标识回到层级里；`resetState` 删掉该 key 的同时把内存里的 viewMode 置回 grid。新建文件夹、打开文件夹、导航、多选链开头断言 grid 模式，不满足时以 `library-view-mode-not-grid` 失败，不得自适应到 list 行继续。

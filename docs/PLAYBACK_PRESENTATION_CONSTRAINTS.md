...
- **佩戴者从 window bar 关掉主窗口等同于按顶部 chrome 的返回**：播放停止、进度落盘，而不是继续在后台出声。visionOS 26 上点 x 默认**不销毁** scene：主窗口 scene 只转入 `.background`、session 保留，app 随后被挂起，`UIScene.didDisconnectNotification` 永不到来，从 Home 再点 app 图标时同一个 scene 原样回到前台（2026-09-08 真机：关窗后只有 `mainScenePhase active -> background`，再开时没有 `mainWindowScene appeared`、residency revision 不变，播放页连同视频一起回来）。所以主窗口在承载播放期间把 `destructionConditions` 设为 `[.userInitiatedDismissal]`（`SpatialPlatformMainWindowDestructionPolicy`，由 `showsWindowPlayback` 驱动、`recordWindowScene` 时重放），点 x 才真的销毁 scene、发出断开通知；执行器按 `recordWindowScene` 记下的 session identifier 匹配断开，清掉主窗口 scene、residency 记 `.closed`、回调 `onMainWindowClosedByWearer`，`SpatialPlatformMainWindowClosurePolicy` 只在有活动播放请求且主窗口正承载播放（已落定的 window／portal，或转换的任一端在主窗口）时停播，沉浸播放期间主窗口只是被 push 到后台，其 scene 若断开不停播。浏览页不设销毁条件，关掉再打开仍回到原来那一页。SwiftUI 根的 `onDisappear` 在主窗口被 push 到后台时也会到、被销毁时又可能晚好几秒（模拟器 23 s），不能当关闭信号。关掉后 app 没有前台 scene，无法再开窗口（`activateSceneSession` 从该状态申请无效，2026-09-08 真机），所以“关窗后把浏览页开回来”做不到；`windowDismissBehavior` 在 visionOS 不可用，也拦不住 x。这条处理在双窗口时期由 508a735e 引入、随 2f1650f7 合并为单窗口时被一并删掉，2026-09-08 按单窗口重建。DEBUG 测试通道的 `closeMainWindow` 用 `requestSceneSessionDestruction` 销毁前台主窗口 session，走同一条断开通知，供 runner 验证这条路径。
- **被销毁的主窗口 scene，其 SwiftUI 视图树不会被拆掉**：scene 销毁后 app 立刻挂起，SwiftUI 根的 `onDisappear` 永远不来；从 Home 再点 app 图标建出新 scene 后，旧树仍随 Observable 状态一起更新（2026-09-08 模拟器：重开后打开视频，`windowSurfaceAttached`、字幕实体、`timer-scheduled`、`windowColliderInstall` 全部成双，`mainWindowScene disappeared` 始终没有出现；真机上表现为视频帧随控件显隐整块消失再出现、docked 二级菜单 hover 忽明忽暗）。两棵树各有一个窗口 RealityView 争同一个共享视频实体、各有一套控件计时器与 ornament 高度回报。主窗口内容外面套一层 `MainWindowSceneGate`：它用 `windowSceneReporting` 记下自己 scene 的 session identifier，coordinator 的 `liveMainWindowSessionIdentifier` 在 `recordWindowScene(.main)` 记下最新连接的 scene（不在 `connectedScenes` 里的 scene 一律忽略）、断开时清空；`SpatialPlatformMainWindowScenePolicy.isOrphaned` 为真的树渲染 `Color.clear`，其 RealityView、ornament、计时器与 `.main` 执行器随之卸载。卸载时 `PlaybackVideoSurface.releaseSurface` 只在实体的 parent 就是本视图记下的 RealityView 根（`hostsEntity`）时才 `removeFromParent` 并释放渲染器消费者，否则只清自己的观察者，不碰别的树正在承载的实体。

...
- **窗口视频网格与 Window 的 SwiftUI 平面共面**，并带 `ModelSortGroup.planarUIInline`——按 z 排序而不是按视图树。画在视频之上的 chrome 因此需要一个向前的位移，否则网格赢下平局、chrome 永不出现。这个位移会让视图离开窗口的 2D 裁切：满宽的矩形前移后四个直角原样露出，缩进躲圆角又留下没有衬托的边，手写圆角半径则永远对不上系统管理的窗口圆角，`ContainerRelativeShape()` 推断的各角半径在真机上左圆右直（2026-09-07 四种都试过）。SwiftUI 材质更不是出路：它采样窗口后面的环境与同层之下的 SwiftUI 内容，采样不到 RealityKit 画的视频，前移时是一块发白的平板，不前移时被网格压住。Apple 对共面内容的规则（`ModelSortGroup.PlanarUIPlacement` 文档）是：RealityKit 网格放在 `depth: 0`、用 `planarUIInline` 按 order 与 SwiftUI 层排序，SwiftUI 前景层只偏 `.ulpOfOne`。但 RealityView 里加的任何网格都拿不到窗口圆角——只有 VideoPlayerComponent 自己画的屏幕有——共面的一块面在真机上仍是直角（2026-09-07）。`.plain` 窗口对 SwiftUI 内容也**不做任何圆角裁切**（不前移的满幅视图在真机上四角照样是直的，2026-09-07）：浏览页的圆角来自自绘玻璃，视频的圆角来自 VideoPlayerComponent 自己画的屏幕。窗口圆角唯一的合法入口是 `ContainerRelativeShape`，它按视图相对容器的偏移推各角半径，只有挂在**窗口根**上（与 `enchronWindowGlassBackground` 同一层）偏移才是零；挂在播放表面那一层时左圆右直（2026-09-07 真机）。因此衬托条 `PlaybackChromeBackdropView` 由 `MainView` 在窗口根上作为 overlay 铺放、以 `ContainerRelativeShape()` 裁切、不前移；要让它压过共面的视频网格，靠的是给视频实体 `ModelSortGroup.planarUIAlwaysBehind`：网格总在共面 SwiftUI 层之后绘制。`PlaybackChromeBackdropSampler` 在窗口 RealityView 每帧取渲染器当前显示的帧顶部一条（`PlaybackEdge.depth` ÷ 画面高）、缩到 160 px、高斯模糊、把前 `holdFraction` 保持、其后 smoothstep 缓入缓出到 0 的 alpha（峰值 `peakOpacity`）烘进去，经 `PlaybackSessionModel.windowChromeBackdropImage` 交给 `PlaybackChromeBackdropView` 在窗口根上满幅铺放。字幕平面仍是 `planarUIInline` order 1，在视频之上。

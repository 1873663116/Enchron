...
- **从空间回到主窗口的 cutover 还要等主窗口回到前台**。resident 窗口 dismiss 后主窗口走系统还原动画，scene 从 background 经 inactive 到 active 约 1–2 s；窗口 RealityView 在这之前就能 attach 并证明像素（2026-09-07 真机：cutover 12:26:31、scene active 12:26:33），若此时切换，RealityView 还不出画，佩戴者看到空窗口停留、视频再单独淡入、控件再单独出现。executor 在 `waitForMainWindowToBecomeForeground` 之后先做 portal viewport refresh，再记 `presentationTargetWindowIsForeground`，表面与 settlement fallback 都以它为 cutover 的前提；cutover 后窗口视频实体 0.001→1 按 `targetFadeDuration` 淡入，chrome 随 transition 结束紧跟其后。主窗口的 glass 在这条边上全程不显示（`WindowGlassPolicy.isRevealingMainWindow`），还原动画里窗口是透明的，佩戴者看到的第一帧就是视频。曾用渲染器最后显示的像素做一张 SwiftUI 静帧桥接（7d80aba1）：静帧是直角矩形、贴在窗口平面，而 portal 视频在圆角门内、位于窗口平面之后，两者不可能重合，佩戴者看到的是一张图片先出现再消失、窗口才出现（2026-09-07 真机），故删除，宁可透明也不放不属于窗口的画面。离场的球面／屏幕连同最后一帧留在空间里直到空间关闭（释放所有权时只摘交互面，不再 `removeFromParent`），关闭动画收走的是画面而不是空场景。

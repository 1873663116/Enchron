...
- **Panorama 的字幕挂在头部锚点下**。全景视频实体在原点，没有可以贴字幕的平面；`ImmersiveSpaceView` 在 panorama 下把字幕实体挂到 `AnchorEntity(.head, trackingMode: .continuous)` 上，`PlaybackSubtitlePlacement` 用 `panoramaScreenSize`（16:9，高 1.6 m）与 `panoramaScreenCenter`（前方 3 m、低 0.3 m）代替视频尺寸与 `planeLift`，字幕因此随头部转动保持在视野内。锚点只在 panorama 时加入 RealityView，其余呈现移除。断言在 `PlaybackPresentationStateTests.subtitleSurfaceBlendsOverTheVideoWithoutOwningDepth`。

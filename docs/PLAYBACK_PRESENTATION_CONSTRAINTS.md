...
- **窗口视频网格与 Window 的 SwiftUI 平面共面**，并带 `ModelSortGroup.planarUIInline`——按 z 排序而不是按视图树。画在视频之上的 chrome 因此需要一个向前的位移，否则网格赢下平局、chrome 永不出现。这个位移会让视图离开窗口的 2D 裁切：满宽的矩形前移后四个直角原样露出，缩进躲圆角又留下没有衬托的边，手写圆角半径则永远对不上系统管理的窗口圆角（2026-09-07 之前三种坏形状都出现过）。前移的视图因此要自己按 `ContainerRelativeShape()` 裁：它解析为窗口自身的圆角形状，按视图相对容器的位置推出各角半径，不写数值。顶部衬托 `PlaybackEdgeEmphasis` 现在是 `.thickMaterial` 满宽贴顶、竖向按 smoothstep 缓出的遮罩（`PlaybackEdge.holdFraction` 之前保持不透明），再以容器形状裁切；两侧不再淡出、不再缩进。

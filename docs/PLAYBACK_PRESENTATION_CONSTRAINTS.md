...
- **从空间回到主窗口的 cutover 还要等主窗口回到前台**。resident 窗口 dismiss 后主窗口走系统还原动画，scene 从 background 经 inactive 到 active 约 1–2 s；窗口 RealityView 在这之前就能 attach 并证明像素（2026-09-07 真机：cutover 12:26:31、scene active 12:26:33），若此时切换，静帧被撤、RealityView 却还不出画，佩戴者看到帧闪一下、空窗口停留、视频再淡入。executor 在 `waitForMainWindowToBecomeForeground` 之后先做 portal viewport refresh，再记 `presentationTargetWindowIsForeground`，表面与 settlement fallback 都以它为 cutover 的前提。cutover 之后是交叉淡入：窗口视频实体 0.001→1 与 bridge 静帧 1→0 同按 `targetFadeDuration`，`PortalExitBridgeView` 自持静帧直到淡出完成，executor 清掉模型里的静帧不会让它中途消失。静帧按窗口比例居中裁切（`scaledToFill`），不再是带黑边的整幅左眼。主窗口的 glass 在这条边上全程不显示（`WindowGlassPolicy.isRevealingMainWindow`），还原动画里只有静帧。离场的球面／屏幕连同最后一帧留在空间里直到空间关闭（释放所有权时只摘交互面，不再 `removeFromParent`），关闭动画收走的是画面而不是空场景。

...
- **佩戴者从 window bar 关掉主窗口等同于按顶部 chrome 的返回**：播放停止、进度落盘，而不是继续在后台出声。关闭的信号是主窗口 UIKit scene 的 `UIScene.didDisconnectNotification`，按 `recordWindowScene` 记下的 session identifier 匹配；SwiftUI 根的 `onDisappear` 在主窗口被 push 到后台时也会到、被关闭时又可能晚好几秒，不能当关闭信号。执行器收到断开后清掉主窗口 scene、把 residency 记为 `.closed`、再回调 `onMainWindowClosedByWearer`；`SpatialPlatformMainWindowClosurePolicy` 只在有活动播放请求且主窗口正承载播放（已落定的 window／portal，或转换的任一端在主窗口）时停播，沉浸播放期间主窗口只是被 push 到后台，它的 scene 若断开不停播。这条处理在双窗口时期由 508a735e 引入、随 2f1650f7 合并为单窗口时被一并删掉，2026-09-08 按单窗口重建。DEBUG 测试通道的 `closeMainWindow` 用 `requestSceneSessionDestruction` 销毁前台主窗口 session，走同一条断开通知，供 runner 验证这条路径。

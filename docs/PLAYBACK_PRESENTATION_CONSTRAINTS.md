...
- **Panorama 的字幕画在一块虚拟屏上**。全景视频实体在原点、朝向单位，没有可以贴字幕的平面；`PlaybackSubtitlePlacement` 在 panorama 下用 `panoramaScreenSize`（16:9，高 1.6 m）与 `panoramaScreenCenter`（前方 3 m、低 0.3 m）代替视频尺寸与 `planeLift`，其余比例换算与窗口相同。断言在 `PlaybackPresentationStateTests.subtitleSurfaceBlendsOverTheVideoWithoutOwningDepth`。

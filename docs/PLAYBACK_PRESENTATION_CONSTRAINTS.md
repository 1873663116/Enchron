...
- **窗口"已离场"的判定必须把本次启动从未连接过的窗口算作离场**：执行器只在 UIKit scene 连接时记下 session identifier，所以一个从未打开过的窗口既没有 identifier 也没有 residency 记录（nil）。2026-09-05 真机：`windowHasLeft` 把 nil 当作"未关闭"，主窗口→播放窗口的交接在等待播放窗口"离场"时超时，探针 `playbackWindowHandover refused incoming=playback sceneStillConnected`，点视频完全无反应。判定是 `residency != .open`；交接被拒时只有在新窗口已经在前台（`windowScenePresentation == true`）才关旧窗口，否则保留旧窗口，避免把唯一窗口关掉。

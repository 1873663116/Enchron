...
final class PlaybackVerificationModel {
    ...
    private(set) var selectedRoute = PlaybackRoute.ffmpegCompressed
    ...
    init(backend: Backend = .core) {
        ...
        case .appAdapter:
            ...
            selectedRoute = .ffmpegCompressed
        }
    ...
    func open(
        ...
        if backend == .appAdapter, route != .ffmpegCompressed {
            controlError = "Enchron PlaybackRuntime only owns the FFmpeg product route."
            status = .failed(controlError!)
    ...
    func selectRoute(_ route: PlaybackRoute) async {
        ...
        await open(selectedURL, route: route)
    }
    ...
    func reopen() async {
        ...
        await open(url, route: selectedRoute)
    }

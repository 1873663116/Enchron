...
final class CorePlaybackScenario {
    ...
    func run() async {
        ...
        do {
            ...
            for route in selectedRoutes() {
                trace("route=\(route.rawValue) phase=open")
                statusText = "Running \(route.label)"
                do {
                    routes.append(try await run(route: route, fixture: fixture))
                } catch {
                    var failure: [String: Any] = [
                        ...
                        "controlError": playback.controlError ?? "none",
                        "displayedColor": currentDisplayedColor(),
                    ]
                    if let snapshot = playback.debugSnapshot() ?? playback.lastSessionSnapshotForProbe() {
                        failure["snapshotAtFailure"] = snapshotObject(snapshot)
                    }
                    routes.append(failure)
                    await playback.close()
                }
                trace("route=\(route.rawValue) phase=complete passed=\(routes.last?["passed"] as? Bool == true)")
            }
            result["routes"] = routes
            result["completed"] = true
            result["passed"] = routes.allSatisfy { $0["passed"] as? Bool == true }
            result["firstFailure"] = routes.first(where: { $0["passed"] as? Bool != true })?["firstFailure"]
    ...
    private func run(route: PlaybackRoute, fixture: LoadedFixture) async throws -> [String: Any] {
        await playback.open(fixture.sourceURL, route: route)
        try await waitForVisibleProgress(route: route, minimumTime: 0.75)
        ...
        await playback.open(fixture.sourceURL, route: route)
        try await waitForVisibleProgress(route: route, minimumTime: 0.5)
    ...
    private func waitForVisibleProgress(route: PlaybackRoute, minimumTime: Double) async throws {
        ...
        while clock.now < deadline {
            if case .failed(let message) = playback.status {
                throw ScenarioError.route(route, message)
            }
            if let snapshot = playback.debugSnapshot() {
                if let error = snapshot.rendererState?.rendererError {
                    throw ScenarioError.route(route, "video renderer: \(error)")
                }
                if let error = snapshot.audioRendererState?.error {
                    throw ScenarioError.route(route, "audio renderer: \(error)")
                }
        ...
        throw ScenarioError.route(route, "timed out waiting for displayed video and audio renderer progress")
    }
    ...
    private func selectedRoutes() -> [PlaybackRoute] {
        if playback.backend == .appAdapter {
            return [.ffmpegCompressed]
        }
        guard let raw = environment["ENCHRON_L2_ROUTE"],
              let route = PlaybackRoute(rawValue: raw) else {
            return PlaybackRoute.allCases
        }
        return [route]
    }

...
- 组件库中已有的控件直接复用。创建局部占位之前，先直接调用 `PlayerControlDeck`、`VideoCardLarge`、`FolderCard`、`FileListRow`、`MenuItemRow`、`SceneCardMedium`、`ViewModeCapsuleControl`、`MockBreadcrumb`、`PathBreadcrumbMenu`、`SearchInputCapsule`、`FilterPillBar`、`SourceSidebarRow` 及相关组件。

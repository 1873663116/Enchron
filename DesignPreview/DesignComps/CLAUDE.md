...
- `Main Window Preview`：主窗口 Preview，包含 Files / Settings，并通过系统 `TabView` 的 Environment 入口打开 SenseZone Volume。
- `SenseZone Volume Preview`：Environments 目的地的 volumetric `WindowGroup`，直接显示 EnvironmentCard 组件，不渲染主窗口玻璃面板，也不保留 `Tab Bar Ornament`。
...
- 主窗口 Preview：Files / Settings 留在主窗口；Environment 入口打开 SenseZone Volume。SenseZone Volume 只显示 EnvironmentCard 组件，左上返回按钮回到进入前的主窗口目的地。
- SenseZone Volume Preview：Environments 目的地的 volumetric `WindowGroup`，直接显示 EnvironmentCard 组件。
...
- Xcode Canvas 中的主窗口 Preview 不作为跨 `WindowGroup` open / dismiss 的证明表面；EnvironmentCard 审查使用具名 `SenseZone Volume` Preview。

...
### G13. 前后端契约变更须同步
- 涉及远程数据流、UI 数据模型或端点语义的改动，须先更新 `api/openapi.yaml`
- 对应 mock 数据须同步更新到 `api/examples/`
- 变更后须通过 `api/scripts/validate-contract.sh` 验证
- 不可合入绕过契约的单侧（纯前端或纯后端）行为变更
- 协作规则和职责边界详见 `frontend-backend-contract.md`

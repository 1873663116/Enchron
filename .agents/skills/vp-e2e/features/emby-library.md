...
- Emby 滚动视图接受带 `--identifier` 的合成滑动（`Emby-library-list`、`Emby-Home`、`Emby-Detail-<id>` 均已实证）。此前记为「Emby 滑动杀 runner」是误归因：杀会话的是省略 identifier 的滑动，与 Emby 无关，Emby 只是恰好在屏。规则见 [产品事实](../references/product.md)。
- 海报横条只有可视区内的卡可点，靠右的卡 tap 返回 False。
- 系列详情页按设计没有播放按钮，播放入口是选集面板的 `Emby-Episode-<id>` 卡片，点击直接进播放。此前记为「顶部无 identifier 的播放图标点按无效」是误归因：该图标是导航栏 Emby 页签 `Emby-Navigation-Tab`，不在详情页，且已在该页签时再点无反应属正确行为。

...
- 每一步声明驱动方式（`real`、`injected`、`setup` 或 `evidence`，语义见 [SKILL.md](../SKILL.md)）。使用注入时必须写明绕过了什么、因此对哪类缺陷失明。需要人类主观判断的项目不生成 Agent 回归步骤。
...
| 感知 | 人类的主观感受 | Agent 回归不生成 obligation；人工测试者自行判断 |

Agent 回归的成功表达式只引用结构证据和可自动判定的物理证据。由产品代码决定的事实归结构证据；取决于 Apple 黑盒或物理世界的客观事实归物理证据；人的感受归感知记录。感知记录不参与 Agent Oracle，也不会在运行中等待人类裁决。

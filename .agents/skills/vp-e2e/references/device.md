...
两个设备标识都从环境读取，仓库内不留字面量：Xcode destination ID 取 `ENCHRON_TARGET_DEVICE`，CoreDevice ID 取 `ENCHRON_CORE_DEVICE`（实现见 `Scripts/verification/enchron_target.py`，缺席时报错而不是回落到某台头显）。两者的事实来源是 `xcrun devicectl list devices` 的输出；换设备或首次配置时从那里读出并导出这两个变量。
...
```sh
xcrun devicectl device info details --device "$ENCHRON_CORE_DEVICE" --timeout 30
```

返回里出现 `Device State: connected`、`Boot State: booted`、`Developer Mode Status: Enabled` 即已连上；隧道地址形如 `Tunnel IP Address: fde0:...`。此后 `xcrun devicectl list devices` 会把 `$ENCHRON_TARGET_DEVICE` 列为 `connected`。2026-09-03 实测这条命令把掉线的头显在数秒内拉回 `connected`。设备缺席时先跑这一条，连不上是重连没做，不是设备的问题。

## 建立会话

```sh
python3 Scripts/verification/interactive_visionpro_ui.py \
  --device "$ENCHRON_TARGET_DEVICE" \

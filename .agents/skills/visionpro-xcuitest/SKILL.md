...
开始前，先阅读 UI 测试目录内最近的指引，并检查当前 runner、控制器、录屏提取器及其 `--help` 输出。Bundle ID、命令、destination 和证据位置由项目内当前文件负责。
 ...
 回归运行用`Scripts/verification/playback_mode_matrix.py`，单点排查用现场驱动；播放 PASS 必须有像素佐证。
...
**单点排查**与**回归运行**的分界：定位、复现bug使用单点排查，用现场驱动；验证一批已完成的修复是否相互冲突是回归运行，用覆盖矩阵。矩阵的用途与成本在它的启动输出中自述。

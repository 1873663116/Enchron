...
这是整份地图里唯一需要**感知验收**的特性族，因为"颜色对不对"的谓词本身是人的判断。
...
| 物理 | 采集帧与验收参照物的差异在阈值内 | **待建**，参照物机制尚不存在 |
| 感知 | 佩戴者确认色彩正常、立体舒适、包裹角自然 | **待做**，每种解释方式一次 |

## 证明的终态

结构侧：三个脚本零失效，且 parity 矩阵改动前后逐字段无差异。物理侧：`visual.verdict = content`。感知侧：佩戴者一次性确认，确认当时采集的帧成为参照物，此后由机器比对。

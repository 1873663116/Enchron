...
`results.jsonl` 该 cell `verdict: PASS`，`landed` 记录落地呈现（window 或 panorama）。window 落地要求控制串 `videoVisible=true` 且 lifecycle 稳态；panorama 落地要求探针 `settled=true`。40 秒未达任一稳态即失败，证据在 cell 目录的探针摘录与最后控制串。

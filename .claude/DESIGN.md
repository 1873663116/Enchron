...
第 8、9 条与写入侧的 `catalog_write_gate.py` 来自擂台里落选的两份方案：第 8、9 条是 P1（`ef34ff18`）的，第三条是 P2（`09ca523e`）的路径版本。基线是 P3（`0916c9b1`）。P2 同一份里另有一条拦读取的规则，未采纳，因为读 Catalog 不改变任何绑定。三个分支在嫁接后删除，SHA 留在这里。

...
@Test func vrAndDolbyFixturesCreateCompressedSamplesThroughFFmpegBridge() throws {
    ...
    let fixtures = [
        ...
        "DolbyVision/HD/Patterns_Of_Nature_HDR10-P8.1_HD_24_H265-2Mbps_DD+JOC-768Kbps.mp4",
        "DolbyVision/HD/Patterns_Of_Nature_HLG-P8.4_HD_24_H265-2Mbps_DD+JOC-768Kbps.mp4",
    ]
...
@Test func missingAV1ExtradataIsBootstrappedFromSphericalFixtureBitstream() throws {
    let fixture = playbackTestMedia.appendingPathComponent("Panorama/360.mp4")
...
@Test func flacPacketsAreDecodedToInterleavedPCM() throws {
    silenceFFmpegDiagnostics()
    ...
    while true {
        ...
        #expect(streamDescription.mFormatFlags & kAudioFormatFlagIsPacked != 0)
        #expect(streamDescription.mFormatFlags & kAudioFormatFlagIsNonInterleaved == 0)
        ...
        #expect(byteCount == frameCount * Int(streamDescription.mBytesPerFrame))
        ...
        #expect(status == noErr)
        sawNonZeroPCM = sawNonZeroPCM || bytes.contains { $0 != 0 }
    }

    #expect(totalFrameCount == 3_840)
...
@Test func missingVP9ConfigurationIsSynthesizedFromCodecParameters() throws {
    ...
    if VTIsHardwareDecodeSupported(kCMVideoCodecType_VP9) == false {
        #expect(reader == nil)

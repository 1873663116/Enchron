...
nonisolated final class EmbyMediaByteSource: MediaByteSource, @unchecked Sendable {
    ...
    func read(in range: Range<Int64>) async throws -> Data {
        guard range.lowerBound >= 0,
              ...
              let expectedCount = Int(exactly: range.upperBound - range.lowerBound) else {
        ...
        if response.statusCode == 416 {
            ...
            if let length { updateServerLength(length) }
        ...
        guard contentRange.start == range.lowerBound,
              contentRange.end == range.upperBound - 1,

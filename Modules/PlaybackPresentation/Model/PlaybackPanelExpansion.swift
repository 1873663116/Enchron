...
/// of them.
///
/// The panel carried one boolean per expandable block, which made every new block a
/// boolean the others had to stay exclusive with by convention, and it changed the
/// panel's size and its contents on one spring at one instant. What the wearer should
/// see is three ordered steps instead: the contents leave, the empty shell resizes,
/// and the contents return at the new size. That order is a fact about the panel
/// rather than about any one button, so it lives here.
///
/// Nothing here measures time. The view advances a step when the animation it started
/// reports that it finished, so retuning a duration in `DesignTokens` cannot leave the

...
struct EmbyMediaByteSourceTests {
    ...
    func routedRangesCarryToken() async throws {
        ...
        let handle = try await MediaByteStreamEndpoint.shared.resolve(
            source,
            filename: "stream.mkv"
        )
        let lease = try #require(handle.accessLease)
        defer { lease.release() }
        ...
        #expect(source.totalLength == 10)
    }

    @Test("Emby byte reads reject an unsatisfiable server range")
    func unsatisfiableRangeThrows() async throws {
        ...
        await #expect(
            throws: EmbyMediaByteSourceError.unsatisfiableRange(totalLength: 10)
        ) {
            try await source.read(in: 10..<11)
        }
        #expect(source.totalLength == 10)
    }

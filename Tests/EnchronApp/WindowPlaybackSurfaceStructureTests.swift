...
struct WindowPlaybackSurfaceStructureTests {
    ...
    func deckOverlaysRealityKitCanvasOnBothPlatforms() throws {
        ...
        let surfaceSource = try String(
            contentsOf: repositoryRoot
                .appending(path: "Modules/PlaybackPresentation/Views/PlaybackVideoSurface.swift"),
            encoding: .utf8
        )
        let mainViewSource = try String(
            contentsOf: repositoryRoot.appending(path: "Apps/Enchron/MainView.swift"),
            encoding: .utf8
        )
        let geometrySource = try String(
            contentsOf: repositoryRoot.appending(
                path: "Modules/PlaybackPresentation/Model/WindowPlaybackPageGeometry.swift"
            ),
            encoding: .utf8
        )
        let projectSource = try String(
            contentsOf: repositoryRoot.appending(path: "Enchron.xcodeproj/project.pbxproj"),
            encoding: .utf8
        )
        ...
        let enchronMacOSMembership = try sourceRegion(
            named: "Exceptions for \"Modules\" folder in \"EnchronMacOS\" target",
        ...
        #expect(
            enchronMacOSMembership.contains(
                "PlaybackPresentation/Model/WindowPlaybackPageGeometry.swift,"
            )
        )
    }
    ...
    func spatialStopUsesTheCleanupBarrier() throws {
        ...
        let launchSource = try String(
            contentsOf: repositoryRoot
                .appending(path: "Modules/PlaybackFeature/PlaybackLaunchCoordinator.swift"),
            encoding: .utf8
        )
        let runtimeSource = try String(
            contentsOf: repositoryRoot.appending(path: "Modules/PlaybackFeature/PlaybackRuntime.swift"),
            encoding: .utf8
        )
        let immersiveSurfaceSource = try String(
            contentsOf: repositoryRoot
                .appending(path: "Modules/PlaybackPresentation/Scenes/ImmersiveSpaceView.swift"),
            encoding: .utf8
        )
